...
for i in range(1, len(hdates)):
    ...
    for index, row in totbtc[:date].iterrows():
        if index < datetime.strptime(date, "%Y-%m-%d") and index > datetime.strptime(hdates[i-1], "%Y-%m-%d"):
            ds_btwn = days_between(str(index.date()),date)
            h = h.append({'Date': index.strftime("%Y-%m-%d"), 'ds_bfr_hlvng': ds_btwn, 'sf': row['sf'], 'cap': row['cap']}, ignore_index=True)
...
sc = ax.scatter(h['sf'], h['cap'], c=h['ds_bfr_hlvng'], alpha=0.9, cmap='jet')
ax.set_title('Stock-to-Flow & Market Value', fontsize=ls)
ax.set_xlabel('ln(Stock-to-Flow)', fontsize=ls)
ax.set_ylabel('ln(Market Value)', fontsize=ls)
ax.set_yscale('log')
ax.set_xscale('log')
...
plt.yscale('log')
plt.plot(totbtc.index, totbtc['cap']/totbtc['Value'], color='royalblue', label="BTCUSD")
plt.plot(totbtc.index, np.exp(12.7598)*totbtc['sf1y']**4.1167/totbtc['Value'], color='red', label="Expected price based on real stock and flow")
plt.plot(d, ep, color='cornflowerblue', label="Expected price based on calculated stock and flow")
plt.text(1.1, 0.1, 'github.com/pyzhyk/sf', horizontalalignment='right', verticalalignment='center', transform=ax.transAxes, fontsize=20, bbox=dict(facecolor='#e1e1e1', alpha=0.4, edgecolor='black', pad=10.0))
